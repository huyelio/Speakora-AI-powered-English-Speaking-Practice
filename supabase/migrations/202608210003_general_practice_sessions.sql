...
begin
  ...
  if p_difficulty is null or p_difficulty not in ('BEGINNER', 'INTERMEDIATE', 'ADVANCED') then
    raise exception 'Unsupported General difficulty';
  end if;

  if array_length(p_question_ids, 1) <> 5
    or (select count(distinct question_id) from unnest(p_question_ids) as selected(question_id)) <> 5 then
  ...
  if exists (
    ...
    raise exception 'questions must be active General questions for the requested topic and difficulty';
  end if;

  insert into public.practice_sessions(user_id, topic_id, mode, difficulty_level, question_count)
  values (p_user_id, p_topic_id, 'GENERAL', p_difficulty, 5)
...
revoke all on function public.create_general_practice_session(uuid,uuid,text,uuid[]) from public,anon,authenticated;
grant execute on function public.create_general_practice_session(uuid,uuid,text,uuid[]) to service_role;
